...
public class PlayerSetupController : GlobalEventListener
{
    ...
    public override void OnEvent(takeDamageEvent evnt)
    {
        if (BoltNetwork.IsServer)
        {
            ...
            Debug.Log("Ell jugador -" + namePlayers[evnt.damagedBy] + "- ha hecho da�o al jugador -" + namePlayers[evnt.nameDamaged]);
        }
    }
    ...
    public override void OnEvent(damageDoneEvent evnt) //Lo recibe el jugador que ha hecho da�o
    {
        Debug.Log("Dispar�: hizo da�o");
        if (evnt.namePlayer == ComInfo.getPlayerName())
        {
            RoundData.damageInflicted += 500; //SE SUPONE QUE EL DA�O ES 500 SIEMPRE
            Debug.Log("Dispar�: da�o inflingido actual: " + RoundData.damageInflicted);
        }
    }

    public override void OnEvent(damageReceivedEvent evnt) //Lo recibe el jugador que ha hecho da�o
    {
        Debug.Log("Dispar�: recibe da�o");

        if (evnt.namePlayer == ComInfo.getPlayerName())
        {
            RoundData.damageReceived += 500; //SE SUPONE QUE EL DA�O ES 500 SIEMPRE
            Debug.Log("Dispar�: da�o recibido actual: " + RoundData.damageReceived);
        }
    }
